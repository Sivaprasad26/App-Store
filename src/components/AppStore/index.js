...
class AppStore extends Component {
  state = {
    searchInput: '',
    activeTapId: tabsList[0].tabId,
  }

  setActiveTabId = tabId => {
    this.setState({activeTapId: tabId})
  }
  ...
  getActiveTabApps = searchApps => {
    const {activeTapId} = this.state
    const filterApps = searchApps.filter(
      eachSearchApp => eachSearchApp.category === activeTapId,
    )
    return filterApps
  }
  ...
  render() {
    const {searchInput, activeTapId} = this.state
          ...
          <ul className="tab-list">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={activeTapId === eachTab.tabId}
              />
            ))}
          </ul>
